...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        self.sumd = self.sumd + math.sqrt((self.x - xi)**2 + (self.y - yi)**2)
    ...
    def get_distance_from_start(self):
        d = math.sqrt((self.x - self.xi)**2 + (self.y - self.yi)**2)
        return d
    ...
    def closer_to(self, p2, p3):
        d2 = math.sqrt((self.x - p2.x)**2 + (self.y - p2.y)**2)
        d3 = math.sqrt((self.x - p3.x)**2 + (self.y - p3.y)**2)
        if d2 > d3:
            return p3
        else:
            return p2
